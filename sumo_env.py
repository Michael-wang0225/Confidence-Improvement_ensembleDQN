...
class SumoEnv:
    ...
    def reset(self):
        ...
        self.init_ego_position = traci.vehicle.getPosition(self.ego_name)
    ...
    def get_state(self):
        ...
        if self.ego_name not in traci.vehicle.getIDList():
            # get 4 nearest vehicles within 30m
            vehicles = [
                [vehicle,
                 distance.euclidean(traci.vehicle.getPosition(vehicle), self.pt)]
                for vehicle in traci.vehicle.getIDList() if
                distance.euclidean(traci.vehicle.getPosition(vehicle), self.pt) < 30]
            vehicles = sorted(vehicles, key=(lambda x: x[1]))[0:4]
            """
            try:
                vehicles.pop(0)
            except IndexError:
...
"""
            ...
            while len(angle_state) < 5:
                angle_state.append(-200)

            self.state = velocity_state + position_state + angle_state
            ...
            vehicles = [
                ...
                distance.euclidean(traci.vehicle.getPosition(vehicle),
                                   traci.vehicle.getPosition(self.ego_name)) < 30]

            vehicles = sorted(vehicles, key=(lambda x: x[1]))[0:5]
            # speed of ego and other 4 vehicles as state, when lack add -200
            velocity_state = [traci.vehicle.getSpeed(vehicle[0]) for vehicle in vehicles]
            while len(velocity_state) < 5:
                velocity_state.append(-200)
    ...
    def get_reward(self):
        ...
        # speed_reward = 0 if ego_speed < 8.33 else ( # if 11.11< speed < 8.33, penalty will be 1, otherwise penalty will be 0
        #     1 if ego_speed < 11.11 else 0)
        # action reward
